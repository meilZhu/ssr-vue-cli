...
module.exports = (cb) => {
  ...
  var mfs = new Mfs();
  webpackComplier.outputFileSystem = mfs;   //修改了webpack的输出形式（改成输出到缓存中），
  ...
  webpackComplier.watch({}, async (error, stats) => {
    ...
    // server Bundle json文件   //然后在从缓存中拿到该文件
    ...
    let clientBundle = await axios.get(`http://localhost:${PORT}/vue-ssr-client-manifest.json`)
